...
function Screen( aoz, renderer, contextName, definition )
{
	...
	if ( this.aoz.platform == 'amiga' && this.numberOfColors == 64 || this.pixelMode.indexOf( 'hbm' ) >= 0 )
	{
	...
	// For AMAL
	this.newPosition = { x: this.position.x, y: this.position.y };
	this.newDisplay = { width: this.display.width, height: this.display.height };
	this.newOffset = { x: this.offset.x, y: this.offset.y };
};
...
///////////////////////////////////////////////////////////////////////////
Screen.getThis = function( index ) { return this.aoz.getScreen( index ) };
Screen.getX = function() 
{ 
	var x = this.newPosition.x;
	if ( typeof x != 'undefined' )
	{
		this.newPosition.x = undefined;
		return x;
	}
	return undefined; 
};
Screen.getY = function() 
{ 
	var y = this.newPosition.y;
	if ( typeof y != 'undefined' )
	{
		this.newPosition.y = undefined;
		return y;
	}
	return undefined; 
};
Screen.setX = function( value ) { this.position.x = value; this.setModified(); };
Screen.setY = function( value ) { this.position.y = value; this.setModified(); };
Screen.getXSize = function() 
{ 
	var width = this.newDisplay.width;
	if ( typeof width != 'undefined' )
	{
		this.newDisplay.width = undefined;
		return width;
	}
	return undefined; 
};
Screen.getYSize = function() 
{ 
	var height = this.newDisplay.height;
	if ( typeof height != 'undefined' )
	{
		this.newDisplay.height = undefined;
		return height;
	}
	return undefined; 
};
Screen.setXSize = function( value ) { this.display.x = value; this.setModified(); };
Screen.setYSize = function( value ) { this.display.y = value; this.setModified(); };
Screen.getXOffset = function() 
{ 
	var offset = this.newOffset.x;
	if ( typeof offset != 'undefined' )
	{
		this.newOffset.x = undefined;
		return offset;
	}
	return undefined; 
};
Screen.getYOffset = function() 
{ 
	var offset = this.newOffset.y;
	if ( typeof offset != 'undefined' )
	{
		this.newOffset.y = undefined;
		return offset;
	}
	return undefined; 
};
Screen.setXOffset = function( value ) { this.offset.x = value; this.setModified(); };
Screen.setYOffset = function( value ) { this.offset.y = value; this.setModified(); };
...
Screen.prototype.setDualPlayfield = function( screen )
{
	...
	this.aoz.screensContext.moveBefore( this.aoz.currentContextName, screen, this );
	this.setModified();
	screen.setModified();
};
...
Screen.prototype.setDisplay = function( rectangle, fromInstruction )
{
	...
	if ( fromInstruction )
	{
		this.newPosition.x = rectangle.x;
		this.newPosition.y = rectangle.y;
		this.newDisplay.width = rectangle.width;
		this.newDisplay.height = rectangle.height;
	}
	this.setModified();
};
...
Screen.prototype.setOffset = function( offset, fromInstruction )
{
	...
	if ( fromInstruction )
	{
		this.newOffset.x = this.offset.x;
		this.newOffset.y = this.offset.y;
	}
	this.setModified();
};
...
{
	var x = Math.floor( ( position.x - this.position.x ) / this.renderScale.x );
	var y = Math.floor( ( position.y - this.position.y ) / this.renderScale.y );
	...
	for ( var p = 0; p < palette.length; p++ )
	{
		if ( typeof palette[ p ] != 'undefined' )
		{
			if ( typeof palette[ p ] == 'string' )
				...
				this.palette[ p ] = this.utilities.getModernColorString( palette[ p ], this.aoz.useShortColors );
...
Screen.prototype.setColour = function( number, color )
{
	if ( this.aoz.usePalette )
	{
		...
		this.palette[ number ] = this.utilities.getModernColorString( color, this.aoz.useShortColors );
...
Screen.prototype.setTransparent = function( colors, trueFalse )
{
	...
	{
		sourceColors[ c ] = this.aoz.utilities.getRGBAStringColors( this.palette[ colors[ c ] % this.numberOfColors ] );
		if ( trueFalse )
		{
			alphas[ c ] = 0;
...
Screen.prototype.createPattern = function( pattern )
{
	...
	var colorBack = this.utilities.getRGBAStringColors( this.getColorString( this.patternInk ? this.patternInk : 0 ) );
...
Screen.prototype.destroyBob = function( index, contextName )
{
	...
	{
		this.aoz.removeRootObjectFromSynchro( this.bobsContext.getElement( contextName, index ) );
...
{
	contextName = typeof contextName == 'undefined' ? this.aoz.currentContextName : contextName;
	return this.aoz.fp2Int( this.bobsContext.getElement( contextName, index, 'bob_not_defined' ).vars.X );
};
Screen.prototype.yBob = function( index, contextName )
{
	contextName = typeof contextName == 'undefined' ? this.aoz.currentContextName : contextName;
	return this.aoz.fp2Int( this.bobsContext.getElement( contextName, index, 'bob_not_defined' ).vars.Y );
};
Screen.prototype.iBob = function( index, contextName )
{
	contextName = typeof contextName == 'undefined' ? this.aoz.currentContextName : contextName;
	return this.bobsContext.getElement( contextName, index, 'bob_not_defined' ).vars.Image;
};
...
Screen.prototype.bobRotate = function( index, angle, tags, contextName )
{
	contextName = typeof contextName == 'undefined' ? this.aoz.currentContextName : contextName;
	var bob = this.bobsContext.getElement( contextName, index, 'bob_not_defined' );
	bob.setAngle( { x:0, y: 0, z: angle }, true );
	this.bobsToUpdate = true;
	this.setModified();
};
...
Screen.prototype.getImage = function( bankName, index, rectangle, tags, contextName )
{
	...
	var remap = ( this.utilities.getTag( tags, [ 'opaque' ] ) ) == '';
	...
	if ( colortag )
	{
		if ( colortag >= 0 && colortag < this.palette.length )
		{
			rgb = this.utilities.getRGBAStringColors( this.palette[ colortag ] );
			remap = true;
		}
	}
	if ( remap && bankName != 'icons' )
	{
		var rgb = this.utilities.getRGBAStringColors( this.palette[ 0 ] );
...
Screen.prototype.putBob = function( index, contextName )
{
	...
	this.paste( 'images', bob.vars.Image, { x: bob.vars.X, y: bob.vars.Y, z: bob.vars.Z }, bob.scale, bob.vars.Angle );
	if ( bob.clipping )
		this.context.restore();
};
...
Screen.prototype.formatText = function( text, position, dimension, align )
{
	if ( !this.font )
		throw 'font_not_available';
	...
	var color = this.getColorString( this.ink );
	var alpha = this.getColorAlpha( this.ink );
	return format( text, position.x, position.y, dimension.width, dimension.height, textAlign, textBaseLine );
	...
	function format( text, x, y, w, h, align, baseLine ) 
	{
		...
		var split_lines = function( mw, text ) 
		{
			...
			{
				var width = self.aoz.fonts.getTextLength( new_line + " " + words[ i ], self.font, self.fontHeight );
				if ( width < mw ) 
				{
					new_line += " " + words[i];
				} 
		...
		if ( typeof dimension.height != 'undefined' )
		{
			...
			self.context.font = self.utilities.getFontString( self.font.fontInformation.name, self.fontHeight * self.scale.x, self.font.weight, self.font.italic );
			...
			if ( align == 'center' )
			{
				for (var j = 0; j < lines.length; ++j, ly+=self.fontHeight+spl) 
				{
					var lx = x + w / 2 - self.aoz.fonts.getTextLength( lines[ j ], self.font, self.fontHeight ) / 2;
					fillText(lines[j], lx, ly);
				}
			}
			else if ( align == 'right' )
			{
				for (var j = 0; j < lines.length; ++j, ly+=self.fontHeight+spl) 
				{
					var lx = x + w - self.aoz.fonts.getTextLength( lines[ j ], self.font, self.fontHeight );
					fillText(lines[j], lx, ly);
				}
			}
			else	// left
