...
function Errors( aoz )
{
	...
	this.errors=
	{
		...
		[
			...
			"illegal_function_call:Appel de fonction invalide",
			...
			"type_mismatch:Incompatibilité de type",
			...
			"illegal_text_window_parameter:Paramètre de fenêtre de texte non conforme",
			...
			"image_not_defined:Image non définie",
...
Errors.prototype.getError = function( error, param )
{
	...
	return { number: -1, index: '', message: error.message };
	...
	function getError( errorList, error )
	{
		if ( errorList )
		{
			...
			if ( number >= 0 )
			{
				...
				if ( error.parameter )
					message = self.aoz.utilities.replaceStringInText( message, error.parameter );
