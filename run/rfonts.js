...
Fonts.prototype.bitmapFont = function( fontRef, charWidth, charHeight, imageRef, scaleX, scaleY )
{
...
Fonts.prototype.bitmapText = function( text, x, y, fontRef, fontStyle )
{
	...
	if( image.canvas )
	{
		...
		for( var c = 0; c < text.length; c++ )
		{
			...
			canvas.getContext( '2d' ).drawImage( image.canvas,  a * bFont.width, fontStyle * bFont.height, bFont.width, bFont.height, c * ( bFont.width * bFont.scaleX ), 0, bFont.width * bFont.scaleX, bFont.height * bFont.scaleY );
		}
